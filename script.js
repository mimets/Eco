  ...
  if (d.error) {
    ...
    if (d.needsVerify) {
      errEl.innerHTML += `<br><button class="link-btn"
        onclick="resendVerify('${identifier}')">
...
async function doRegister(e) {
  ...
  // ✅ Successo — mostra messaggio e blocca
  errEl.style.color = '#16a34a';
  errEl.innerHTML   = `
    ✅ Registrazione completata!<br>
    📧 Controlla <b>${email}</b> per verificare l'account.
  `;
...
function drawMii(state, canvasId, size = 120) {
  ...
  switch (state.eyes) {
    ...
    case 'cool':
      ...
      ctx.fillRect(eyeXL - eyeR*0.1, eyeY - eyeR*0.7, unit*2, eyeR*1.4);
      break;
    case 'star':
      [eyeXL, eyeXR].forEach(ex => {
...
function pickMii(cat, val, el) {
  miiState[cat] = val;
  el.closest('.mii-opts')?.querySelectorAll('.mii-opt')
...
async function buyItem(itemId) {
  ...
  showConfirm(
    'Acquisto',
    `Acquistare "${item.name}" per ${item.cost} punti?`,
    async () => {
    ...
    return `
      <div class="challenge-item ${expired?'expired':''}">
        <div class="ch-header">
          ...
          ${c.is_public ? '<span class="pill pill-green">🌍 Pubblica</span>'
        ...
        <div class="post-card" id="post_${p.id}">
          <div class="post-header">
            ...
            <div class="post-meta">
              ...
              <div class="post-time">@${p.author_username||''} · 
                ${timeAgo(new Date(p.created_at))}
              </div>
            </div>
            ${p.author_id === myProfile?.id
              ? `<button class="btn-icon danger-btn ml-auto"
          ...
          ${p.image_url
            ? `<img src="${p.image_url}" class="post-img" alt="post"
                 onerror="this.style.display='none'">`
            : ''
          }
          <div class="post-actions">
            <button class="btn-like ${p.liked_by_me?'liked':''}"
        ...
        <div class="comment-item">
          ...
          ${c.author_id === myProfile?.id
            ? `<button class="btn-icon danger-btn"
                 onclick="deleteComment(${postId},${c.id})">✕</button>`
            : ''
          }
        </div>`).join('')}
...
async function addComment(postId) {
  ...
  el.value = '';
  await toggleComments(postId); // chiude
  await toggleComments(postId); // riapre aggiornato
...
async function loadAdmin() {
  if (!myProfile?.is_admin) {
    document.getElementById('admin').innerHTML =
      '<div class="empty"><div class="ei">🚫</div><p>Accesso negato.</p></div>';
    return;
  }
  ...
  if (sEl && !stats.error) {
    sEl.innerHTML = `
      ...
      <div class="admin-stat-card">
        ...
        <div class="asc-label">☁️ kg CO₂ totale</div>
      </div>
      <div class="admin-stat-card">
        ...
        <tbody>
          ${activities.map(a => `
            <tr>
              ...
              <td>
                <button class="btn-sm btn-danger"
                  onclick="adminDeleteActivity(${a.id})">
                  🗑️
                </button>
              </td>
            </tr>`).join('')}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('actForm')
    ?.addEventListener('submit', logActivity);
  ...
  document.getElementById('pwdForm')
    ?.addEventListener('submit', changePassword);
  document.getElementById('forgotForm')
    ?.addEventListener('submit', doForgotPassword);
  document.getElementById('resetForm')
    ?.addEventListener('submit', doResetPassword);

  // mobile nav close on overlay click
