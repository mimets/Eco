...
async function doLogin(e) {
  ...
  const identifierEl = document.getElementById('lIdentifier') || document.getElementById('lEmail');
  ...
  if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-sign-in-alt"></i> <span>Accedi</span>'; }

  if (d.error) {
    errEl.textContent = d.error;
    if (d.needsVerify) {
      errEl.innerHTML += `<br><button class="link-btn" onclick="resendVerify('${identifier}')">📧 Reinvia email verifica</button>`;
    }
    return;
  }

  await doLoginSuccess(d);
}
window.doLogin = doLogin;

async function doLoginSuccess(d) {
  token = d.token;
...
async function doRegister(e) {
  ...
  const password = document.getElementById('rPwd')?.value?.trim();
  ...
  // ✅ FIX: auto-login dopo registrazione
  errEl.style.color = '#16a34a';
  errEl.textContent = '✅ Registrazione completata! Accesso in corso...';

  const loginData = await api('/api/login', 'POST', { identifier: email, password });
  if (loginData.token) {
    await doLoginSuccess(loginData);
  } else {
    errEl.textContent = '✅ Registrato! Ora accedi manualmente.';
    switchTab('login');
  }
}
...
function updateSidebar(u) {
  ...
  const p = document.getElementById('sbPoints');
  const c = document.getElementById('sbCo2');
  if (n) n.textContent = u.name     || '';
  if (e) e.textContent = u.email    || '';
  if (p) p.textContent = (u.points  || 0) + ' pts';
  if (c) c.textContent = parseFloat(u.co2_saved || 0).toFixed(1) + ' kg';
...
function drawMii(state, canvasId, size = 120) {
  ...
  switch (state.hair) {
    ...
    case 'long':
      ctx.beginPath(); ctx.ellipse(cx, cy, r*1.05, r*1.3, 0, Math.PI*1.1, Math.PI*1.9);
    ...
    case 'bun':
      ctx.beginPath(); ctx.arc(cx, cy - r*1.15, r*0.35, 0, Math.PI*2);
    ...
    case 'cap':
      ctx.beginPath(); ctx.ellipse(cx, cy - r*0.55, r*1.1, r*0.25, 0, 0, Math.PI*2);
      ctx.fillStyle = '#1d4ed8'; ctx.fill();
      ctx.beginPath(); ctx.arc(cx, cy - r*0.6, r*0.9, Math.PI, 0);
    ...
    case 'gold':
      ...
      ctx.beginPath(); ctx.arc(cx, cy - r*0.1, r*0.95, Math.PI, 0);
      ctx.fillStyle = '#fcd34d'; ctx.fill();
      ctx.shadowBlur = 0; break;
    case 'galaxy':
      ctx.beginPath(); ctx.arc(cx, cy - r*0.1, r, Math.PI, 0);
      const gGrad = ctx.createLinearGradient(cx-r, cy-r, cx+r, cy);
      gGrad.addColorStop(0,'#4c1d95'); gGrad.addColorStop(0.5,'#7c3aed'); gGrad.addColorStop(1,'#ec4899');
      ctx.fillStyle = gGrad; ctx.fill();
      for (let i = 0; i < 8; i++) {
        ctx.beginPath();
        ctx.arc(cx - r*0.7 + Math.random()*r*1.4, cy - r*0.8 + Math.random()*r*0.6, size*0.018, 0, Math.PI*2);
        ctx.fillStyle = 'white'; ctx.fill();
      } break;
    case 'flame':
      ctx.beginPath(); ctx.arc(cx, cy - r*0.1, r, Math.PI, 0);
      const fGrad = ctx.createLinearGradient(cx, cy-r*1.2, cx, cy-r*0.1);
      fGrad.addColorStop(0,'#fbbf24'); fGrad.addColorStop(0.5,'#f97316'); fGrad.addColorStop(1,'#dc2626');
  ...
  switch (state.eyes) {
    case 'happy':
      [eyeXL, eyeXR].forEach(ex => {
        ctx.beginPath(); ctx.arc(ex, eyeY + eyeR*0.5, eyeR*1.1, Math.PI, 0, true);
        ctx.strokeStyle = '#1e293b'; ctx.lineWidth = unit*2.5; ctx.stroke();
      }); break;
    case 'sleepy':
      [eyeXL, eyeXR].forEach(ex => {
        ctx.beginPath(); ctx.arc(ex, eyeY, eyeR, 0, Math.PI);
    ...
    case 'surprised':
      [eyeXL, eyeXR].forEach(ex => {
        ...
        ctx.beginPath(); ctx.arc(ex, eyeY, eyeR*0.7, 0, Math.PI*2);
        ctx.fillStyle = '#1e293b'; ctx.fill();
      }); break;
    case 'wink':
      ctx.beginPath(); ctx.arc(eyeXL, eyeY, eyeR, 0, Math.PI*2);
      ctx.fillStyle = '#1e293b'; ctx.fill();
      ctx.beginPath(); ctx.arc(eyeXR, eyeY + eyeR*0.5, eyeR*1.1, Math.PI, 0, true);
      ctx.strokeStyle = '#1e293b'; ctx.lineWidth = unit*2.5; ctx.stroke(); break;
    case 'cool':
      ctx.fillStyle = '#1e293b';
      ctx.beginPath(); ctx.roundRect(eyeXL - eyeR*1.5, eyeY - eyeR*0.7, eyeR*3, eyeR*1.4, eyeR*0.4); ctx.fill();
      ctx.beginPath(); ctx.roundRect(eyeXR - eyeR*1.5, eyeY - eyeR*0.7, eyeR*3, eyeR*1.4, eyeR*0.4); ctx.fill();
      ctx.fillStyle = '#0f172a'; ctx.fillRect(eyeXL - eyeR*0.1, eyeY - eyeR*0.7, unit*2, eyeR*1.4); break;
    case 'star':
      [eyeXL, eyeXR].forEach(ex => {
        ctx.fillStyle = '#fbbf24'; ctx.font = `${eyeR*2.2}px serif`;
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        ctx.fillText('⭐', ex, eyeY);
      }); break;
    case 'heart':
      [eyeXL, eyeXR].forEach(ex => {
        ctx.fillStyle = '#ef4444'; ctx.font = `${eyeR*2.2}px serif`;
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ...
    case 'laser':
      [eyeXL, eyeXR].forEach(ex => {
        ...
        ctx.beginPath(); ctx.arc(ex, eyeY, eyeR*0.4, 0, Math.PI*2);
    ...
    default:
      [eyeXL, eyeXR].forEach(ex => {
        ctx.beginPath(); ctx.arc(ex, eyeY, eyeR, 0, Math.PI*2);
        ctx.fillStyle = '#1e293b'; ctx.fill();
        ctx.beginPath(); ctx.arc(ex + eyeR*0.3, eyeY - eyeR*0.3, eyeR*0.3, 0, Math.PI*2);
  ...
  switch (state.mouth) {
    case 'grin':
      ctx.beginPath(); ctx.arc(cx, mouthY - r*0.08, r*0.38, 0.15, Math.PI - 0.15);
      ctx.strokeStyle = '#1e293b'; ctx.lineWidth = unit*2.5; ctx.stroke();
      ctx.beginPath(); ctx.arc(cx, mouthY - r*0.08, r*0.38, 0.15, Math.PI - 0.15);
      ctx.lineTo(cx - r*0.37, mouthY - r*0.08);
      ctx.fillStyle = '#fca5a5'; ctx.fill(); break;
    case 'open':
      ctx.beginPath(); ctx.ellipse(cx, mouthY, r*0.22, r*0.15, 0, 0, Math.PI*2);
    ...
    case 'smirk':
      ...
      ctx.strokeStyle = '#1e293b'; ctx.lineWidth = unit*2.5; ctx.stroke(); break;
    case 'tongue':
      ctx.beginPath(); ctx.arc(cx, mouthY - r*0.05, r*0.3, 0, Math.PI);
      ctx.fillStyle = '#1e293b'; ctx.fill();
      ctx.beginPath(); ctx.ellipse(cx, mouthY + r*0.12, r*0.16, r*0.13, 0, 0, Math.PI*2);
      ctx.fillStyle = '#f87171'; ctx.fill(); break;
    case 'sad':
      ctx.beginPath(); ctx.arc(cx, mouthY + r*0.25, r*0.32, Math.PI + 0.3, -0.3);
      ctx.strokeStyle = '#1e293b'; ctx.lineWidth = unit*2.5; ctx.stroke(); break;
    case 'rainbow':
      ['#ef4444','#f97316','#eab308','#22c55e','#3b82f6'].forEach((c,i) => {
        ctx.beginPath(); ctx.arc(cx, mouthY - r*0.05, r*(0.28 + i*0.04), 0.2, Math.PI - 0.2);
        ctx.strokeStyle = c; ctx.lineWidth = unit*2; ctx.stroke();
      }); break;
    case 'fire':
      ctx.font = `${r*0.55}px serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText('🔥', cx, mouthY + r*0.05); break;
    default:
      ctx.beginPath(); ctx.arc(cx, mouthY - r*0.05, r*0.32, 0.2, Math.PI - 0.2);
      ctx.strokeStyle = '#1e293b'; ctx.lineWidth = unit*2.5; ctx.stroke();
  }
  ctx.restore();
}
...
function pickMii(cat, val, el) {
  miiState[cat] = val;
  el.closest('.mii-opts')?.querySelectorAll('.mii-opt').forEach(o => o.classList.remove('sel'));
...
async function loadDashboard() {
  ...
  const els = {
    ...
    co2Week:    document.getElementById('co2Week'),
    co2Month:   document.getElementById('co2Month'),
    totalActs:  document.getElementById('totalActs'),
    userPoints: document.getElementById('userPoints'),
  };

  if (els.co2Total)  els.co2Total.textContent  = parseFloat(stats.co2_saved        || 0).toFixed(1);
  if (els.co2Week)   els.co2Week.textContent   = parseFloat(stats.co2_week         || 0).toFixed(1);
  if (els.co2Month)  els.co2Month.textContent  = parseFloat(stats.co2_month        || 0).toFixed(1);
  if (els.totalActs) els.totalActs.textContent = stats.total_activities             || 0;
  if (els.userPoints)els.userPoints.textContent= stats.points                       || 0;
...
function renderYearlyChart(data) {
  ...
  data.forEach((d, i) => {
    ...
    if (parseFloat(d.co2) > 0) {
      ctx.fillStyle = '#22c55e'; ctx.font = '9px Inter, sans-serif';
...
async function logActivity(e) {
  ...
  showN(`✅ +${d.co2_saved} kg CO₂ salvata! +${d.points} punti 🎉`, 'success');
  ...
  updateSidebar({ ...myProfile, points: (myProfile?.points||0) + d.points });
}
...
async function getSuggestions(fieldId, query) {
  ...
  try {
    ...
    const d = await r.json();
    const el = document.getElementById(fieldId + 'Sugg');
    if (!el) return;
    el.innerHTML = d.map(p => `
      <div class="addr-item" onclick="selectAddr('${fieldId}','${p.display_name.replace(/'/g,"\\'")}')">
        📍 ${p.display_name}
      </div>`).join('');
  } catch {}
}
...
async function loadShop() {
  const [items, profile] = await Promise.all([api('/api/shop'), api('/api/profile')]);
  ...
  const catLabels = { hair:'💇 Capelli', eyes:'👀 Occhi', mouth:'👄 Bocca', color:'🎨 Colore', skin:'👤 Pelle' };
            ...
            return `
              <div class="shop-item ${owned?'owned':''} ${item.is_rare?'rare':''}" onclick="openShopPreview(${item.id})">
                ...
                <div class="shop-item-cost">
                  ${owned ? '<span class="owned-badge">✅ Posseduto</span>' : `<span class="pts-badge">⭐ ${item.cost}</span>`}
                </div>
    ...
    return `
      <div class="challenge-item ${expired?'expired':''}">
        <div class="ch-header">
          ...
          ${c.is_public ? '<span class="pill pill-green">🌍 Pubblica</span>' : '<span class="pill pill-gray">🔒 Privata</span>'}
        </div>
...
async function createChallenge(e) {
  ...
  const d = await api('/api/challenges', 'POST', { title, description, co2_target, points_reward, end_date, is_public });
  if (d.error) return showN('❌ ' + d.error, 'error');
...
async function loadLeaderboard() {
  ...
  el.innerHTML  = data.map((u, i) => `
    <div class="lb-item ${i < 3 ? 'top' : ''}">
      ...
      <canvas id="lbav_${u.id}" width="40" height="40" style="border-radius:10px;flex-shrink:0"></canvas>
      <div class="lb-info">
...
async function loadSocial() {
  const [posts, users] = await Promise.all([api('/api/social/posts'), api('/api/social/users')]);
      ...
      feed.innerHTML = posts.map(p => `
        <div class="post-card" id="post_${p.id}">
          <div class="post-header">
            <canvas id="pav_${p.id}" width="40" height="40" style="border-radius:10px;flex-shrink:0"></canvas>
            <div class="post-meta">
          ...
          <div class="comments-section" id="comm${p.id}" style="display:none"></div>
        </div>`).join('');
  ...
  if (uList && !users.error) {
    if (!users.length) {
      uList.innerHTML = '<p style="color:var(--text-muted);font-size:13px">Nessun altro utente.</p>';
    } else {
      uList.innerHTML = users.map(u => `
        <div class="user-item">
          <canvas id="uav_${u.id}" width="32" height="32" style="border-radius:8px;flex-shrink:0"></canvas>
          <div class="user-info">
            <div class="user-name">${u.name}</div>
            <div class="user-un">@${u.username}</div>
          </div>
          <button class="btn-follow ${u.following?'following':''}" onclick="toggleFollow(${u.id},this)">
            ${u.following ? '✅' : '➕'}
          </button>
        </div>`).join('');
...
async function createPost(e) {
  ...
  if (btn) { btn.disabled = false; btn.innerHTML = '📤 Pubblica'; }
  if (d.error) return showN('❌ ' + d.error, 'error');
...
async function toggleComments(postId) {
  const el = document.getElementById('comm' + postId);
  if (!el) return;
    ...
    el.innerHTML = d.map(c => `
      <div class="comment-item">
        ...
        ${c.author_id === myProfile?.id
          ? `<button class="btn-icon danger-btn" onclick="deleteComment(${postId},${c.id})">🗑️</button>`
          : ''}
      </div>`).join('') + `
      <div class="comment-form">
        <input id="ci${postId}" class="input-sm" placeholder="Aggiungi un commento..."
...
async function loadProfile() {
  const [profile, badges] = await Promise.all([api('/api/profile'), api('/api/badges')]);
  ...
  setEl('profName',   profile.name);
  setEl('profUser',   profile.username);
  setEl('profEmail',  profile.email);
  setEl('profPoints', profile.points || 0);
  setEl('profCo2',    parseFloat(profile.co2_saved || 0).toFixed(1));
  setEl('profActs',   profile.total_activities || 0);
  setEl('profLevel',  getLevel(profile.points || 0));
  ...
  if (editUser) editUser.value = profile.username || '';
  ...
  document.querySelectorAll('.mii-opt').forEach(o => {
    o.classList.remove('sel', 'locked');
    ...
    if (miiState[cat] === val) o.classList.add('sel');
  });
  ...
  if (bEl && !badges.error) {
    bEl.innerHTML = badges.map(b => `
      <div class="badge-item ${b.unlocked ? 'unlocked' : 'locked'}" title="${b.name}: ${b.desc}">
        <div class="badge-icon">${b.icon}</div>
        <div class="badge-name">${b.name}</div>
...
async function saveAvatar() {
  ...
  updateSidebar({ ...myProfile, ...miiState });
}
...
async function changePassword(e) {
  ...
  const d = await api('/api/profile/password', 'PUT', { current_password: current, new_password: newPwd });
  if (d.error) return showN('❌ ' + d.error, 'error');
...
async function loadAdmin() {
  if (!myProfile?.is_admin) {
    document.getElementById('admin').innerHTML = '<div class="empty"><div class="ei">🚫</div><p>Accesso negato.</p></div>';
    return;
  }

  const [users, stats, activities] = await Promise.all([
    api('/api/admin/users'), api('/api/admin/stats'), api('/api/admin/activities')
  ]);

  const sEl = document.getElementById('adminStats');
  if (sEl && !stats.error) {
    sEl.innerHTML = `
      <div class="admin-stat-card"><div class="asc-val">${stats.total_users||0}</div><div class="asc-label">Utenti</div></div>
      <div class="admin-stat-card"><div class="asc-val">${stats.total_activities||0}</div><div class="asc-label">Attività</div></div>
      <div class="admin-stat-card"><div class="asc-val">${parseFloat(stats.total_co2||0).toFixed(1)}</div><div class="asc-label">kg CO₂ totale</div></div>
      <div class="admin-stat-card"><div class="asc-val">${stats.total_posts||0}</div><div class="asc-label">Post</div></div>`;
  }
  ...
  if (uEl && !users.error) {
    uEl.innerHTML = `
      <table class="admin-table">
        <thead><tr><th>Utente</th><th>Email</th><th>Punti</th><th>CO₂</th><th>Ruolo</th><th>Verif.</th><th>Azioni</th></tr></thead>
        <tbody>
          ${users.map(u => `
            <tr>
              <td><b>${u.name}</b><br>${u.username}</td>
              <td>${u.email}</td>
              <td>${u.points||0}</td>
              <td>${parseFloat(u.co2_saved||0).toFixed(1)} kg</td>
              <td>${u.is_admin ? '<span class="pill pill-gold">Admin</span>' : '<span class="pill pill-gray">User</span>'}</td>
              <td>${u.verified ? '<span class="pill pill-green">✅</span>' : '<span class="pill pill-red">❌</span>'}</td>
              <td class="admin-actions">
                <button class="btn-sm" onclick="adminEditUser(${u.id},'${u.name}','${u.username}',${u.points||0},${u.is_admin?1:0})">✏️</button>
                <button class="btn-sm btn-danger" onclick="adminDeleteUser(${u.id},'${u.name}')">🗑️</button>
                ${!u.verified ? `<button class="btn-sm btn-green" onclick="adminVerifyUser(${u.id})">✅</button>` : ''}
              </td>
            </tr>`).join('')}
        </tbody>
      </table>`;
  ...
  if (aEl && !activities.error) {
    aEl.innerHTML = `
      <table class="admin-table">
        <thead><tr><th>Utente</th><th>Tipo</th><th>Km/Ore</th><th>CO₂</th><th>Punti</th><th>Data</th><th>Azione</th></tr></thead>
        <tbody>
          ${activities.map(a => `
            <tr>
              <td>${a.user_name||''}</td>
              ...
              <td><button class="btn-sm btn-danger" onclick="adminDeleteActivity(${a.id})">🗑️</button></td>
            </tr>`).join('')}
        </tbody>
      </table>`;
  }
...
function adminEditUser(id, name, username, points, isAdmin) {
  showConfirm('Modifica Utente', `Modificare ${name}?`, () => {
    ...
    api(`/api/admin/users/${id}`, 'PUT', { name: newName, username: newUser, points: newPts, is_admin: newAdmin })
      .then(d => {
        if (d.error) return showN('❌ ' + d.error, 'error');
...
async function doForgotPassword(e) {
  ...
  if (btn) { btn.disabled = false; btn.innerHTML = '📧 Invia Link'; }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('actForm')        ?.addEventListener('submit', logActivity);
  document.getElementById('postForm')       ?.addEventListener('submit', createPost);
  document.getElementById('challengeForm')  ?.addEventListener('submit', createChallenge);
  document.getElementById('profileForm')    ?.addEventListener('submit', saveProfile);
  document.getElementById('pwdForm')        ?.addEventListener('submit', changePassword);
  ...
  // mobile overlay
  document.getElementById('sidebarOverlay')?.addEventListener('click', toggleMobNav);
