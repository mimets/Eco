...
async function doForgotPassword(e) {
  ...
  if (btn) {
    btn.disabled = false;
    btn.innerHTML = '<i class="fas fa-paper-plane"></i><span>Invia Link Reset</span>';
  }
...
async function doResetPassword(e) {
  ...
  if (!resetTok) {
    errEl.textContent = 'Token mancante';
    return;
  }
  if (!newPwd) {
...
// Funzione renderYearlyChart FIXATA - non mostra più errore
function renderYearlyChart(data) {
  ...
  // Se non ci sono dati, mostra un messaggio amichevole
  if (!data || data.length === 0) {
    ctx.fillStyle = '#94a3b8';
    ctx.font = 'bold 14px Inter, sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('🌱 Nessuna attività quest\'anno', w / 2, h / 2 - 10);
    ctx.font = '12px Inter, sans-serif';
    ctx.fillText('Registra la tua prima attività per vedere il grafico!', w / 2, h / 2 + 20);

    // Disegna una linea di base tratteggiata
...
async function logActivity(e) {
  ...
  if (btn) {
    btn.disabled = false;
    btn.innerHTML = '<i class="fas fa-plus"></i> Registra';
  }
...
async function createPost(e) {
  ...
  const d = await api('/api/social/posts', 'POST', {
    content,
    image_url: imageUrl
  });
  if (btn) {
...
async function addComment(postId) {
  ...
  if (!content) return;
  const d = await api(`/api/social/posts/${postId}/comments`, 'POST', {
    content
  });
  if (d.error) return showN('❌ ' + d.error, 'error');
  el.value = '';
...
async function buyItem(itemId) {
  ...
  showConfirm('Acquisto', `Acquistare "${item.name}" per ${item.cost} punti?`, async () => {
    const d = await api('/api/shop/buy', 'POST', {
      item_id: itemId
    });
    if (d.error) return showN('❌ ' + d.error, 'error');
...
async function saveProfile(e) {
  ...
  const d = await api('/api/profile', 'PUT', {
    name,
    username,
    bio
  });
      ...
      <table class="admin-table">
        <thead>
          <tr>
            ...
            <th>CO₂</th><th>Ruolo</th><th>Azioni</th>
          </tr>
        </thead>
        <tbody>
          ${users.map(u => `
