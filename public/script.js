...
let miiState = { color:'#16a34a', skin:'#fde68a', eyes:'normal', mouth:'smile', hair:'none' };
...
const ACTIVITY_ICONS = { 'Bici':'🚴','Treno':'🚂','Bus':'🚌','Carpooling':'🚗','Remoto':'🏠','Videocall':'💻' };
const BG_COLORS = ['#16a34a','#22c55e','#3b82f6','#6366f1','#8b5cf6','#ec4899','#ef4444','#f59e0b','#06b6d4','#14b8a6','#84cc16','#f97316','#1e293b','#64748b','#ffffff'];
const SKIN_COLORS = ['#fde68a','#fcd34d','#f6ad55','#ed8936','#c05621','#7b341e','#fef3c7','#ffe4e6'];
...
function showNotification(msg, type='success') {
  ...
  t.textContent=msg; t.className=`notification-toast show ${type}`;
...
function checkPasswordStrength(pw) {
  const ok = { length:pw.length>=8, upper:/[A-Z]/.test(pw), number:/[0-9]/.test(pw), symbol:/[^a-zA-Z0-9]/.test(pw) };
  const map = { strengthLength:ok.length, strengthUpper:ok.upper, strengthNumber:ok.number, strengthSymbol:ok.symbol,
                resetStrengthLength:ok.length, resetStrengthUpper:ok.upper, resetStrengthNumber:ok.number, resetStrengthSymbol:ok.symbol };
  Object.entries(map).forEach(([id,valid]) => { const el=document.getElementById(id); if(el) el.style.color=valid?'#16a34a':'#94a3b8'; });
...
async function apiRequest(endpoint, method='GET', body=null) {
  try {
    ...
    if(res.status===401) {
      token=null; localStorage.removeItem('ecotoken');
      document.getElementById('authContainer').style.display='flex';
      document.getElementById('appContainer').style.display='none';
      showNotification('Sessione scaduta','error');
    }
    return data;
  } catch(err) { console.error('API error:',err); return {error:'Errore di connessione'}; }
}
...
async function handleLogin(e) {
  ...
  btn.disabled=true; btn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Accesso...';
  const data = await apiRequest('/api/login','POST',{identifier,password});
  btn.disabled=false; btn.innerHTML='<i class="fas fa-sign-in-alt"></i> Accedi';
  if(data.error) { showNotification(data.error,'error'); return; }
  token=data.token; myProfile=data.user;
  localStorage.setItem('ecotoken',token);
  document.getElementById('authContainer').style.display='none';
  document.getElementById('appContainer').style.display='flex';
  updateSidebar(data.user); syncMiiState(data.user);
  await loadDashboard(); await loadNotificationCount();
...
async function handleRegister(e) {
  ...
  if(!name||!username||!email||!password) { showNotification('Tutti i campi sono obbligatori','error'); return; }
  if(!checkPasswordStrength(password)) { showNotification('Password non sicura','error'); return; }
  const btn=document.getElementById('registerBtn');
  btn.disabled=true; btn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Registrazione...';
  const data=await apiRequest('/api/register','POST',{name,username,email,password});
  btn.disabled=false; btn.innerHTML='<i class="fas fa-user-plus"></i> Registrati';
  if(data.error) { showNotification(data.error,'error'); return; }
...
async function handleForgotPassword(e) {
  ...
  showNotification('📧 Se l\'email esiste riceverai il link.','success');
...
async function handleResetPassword(e) {
  ...
  if(!checkPasswordStrength(newPw)) { showNotification('Password non sicura','error'); return; }
...
function logout() {
  showConfirm('Logout','Sei sicuro di voler uscire?',()=>{
    token=null; myProfile=null; localStorage.removeItem('ecotoken');
    document.getElementById('authContainer').style.display='flex';
    document.getElementById('appContainer').style.display='none';
    switchAuthTab('login'); showNotification('Arrivederci! 👋','info');
  },'👋');
}
...
async function loadDashboard() {
  ...
  document.getElementById('dashGreeting').textContent=`${greet}, ${myProfile?.name||'utente'}! Continua così 💪`;
  const [stats,activities,yearly,badges]=await Promise.all([
    apiRequest('/api/stats'), apiRequest('/api/activities'),
    apiRequest('/api/yearly'), apiRequest('/api/badges')
  ]);
  if(!stats.error) {
  ...
  if(!activities.error&&activities.length>0) {
    rc.innerHTML=activities.slice(0,5).map(a=>`
      <div class="activity-item">
        ...
        <div class="act-info"><strong>${escapeHtml(a.type)}</strong>
          <small>${a.km?a.km+' km':''}${a.hours?a.hours+' ore':''}${a.note?' · '+escapeHtml(a.note):''}</small>
        </div>
        <div class="act-meta"><div class="act-co2">-${a.co2_saved} kg</div><div class="act-pts">+${a.points} pt</div></div>
      </div>`).join('');
  } else { rc.innerHTML=`<div class="empty-state"><span>🌱</span><p>Nessuna attività ancora</p></div>`; }
  if(!yearly.error&&yearly.length>0) renderYearlyChart(yearly); else renderEmptyChart();
...
function renderYearlyChart(data) {
  const canvas=document.getElementById('yearlyChart'); if(!canvas) return;
  const ctx=canvas.getContext('2d');
  canvas.width=canvas.offsetWidth||600; canvas.height=canvas.offsetHeight||200;
  const values=new Array(12).fill(0);
  const months=['Gen','Feb','Mar','Apr','Mag','Giu','Lug','Ago','Set','Ott','Nov','Dic'];
  data.forEach(item=>{ const idx=months.indexOf(item.month); if(idx>=0) values[idx]=parseFloat(item.co2)||0; });
  const max=Math.max(...values,1), W=canvas.width, H=canvas.height, bw=(W-60)/12;
  ctx.clearRect(0,0,W,H);
  ctx.strokeStyle='#e2e8f0'; ctx.lineWidth=1;
  ctx.beginPath(); ctx.moveTo(40,20); ctx.lineTo(40,H-30); ctx.lineTo(W-10,H-30); ctx.stroke();
  values.forEach((v,i)=>{
    const bh=(v/max)*(H-70), x=45+i*bw, y=H-35-bh;
    const g=ctx.createLinearGradient(0,y,0,H-35);
    g.addColorStop(0,'#16a34a'); g.addColorStop(1,'#86efac');
    ctx.fillStyle=g; ctx.beginPath();
    if(ctx.roundRect) ctx.roundRect(x,y,bw-6,bh,4); else ctx.fillRect(x,y,bw-6,bh);
    ctx.fill();
    ctx.fillStyle='#94a3b8'; ctx.font='10px Inter,sans-serif'; ctx.textAlign='center';
    ctx.fillText(months[i],x+(bw-6)/2,H-15);
    if(v>0) { ctx.fillStyle='#16a34a'; ctx.font='bold 9px Inter,sans-serif'; ctx.fillText(v.toFixed(1),x+(bw-6)/2,y-5); }
  });
}
...
function selectActivityType(type, btn) {
  ...
  if(rate.type==='km') {
    ...
    requestAnimationFrame(()=>{ if(!mapInitialized) initMap(); else mapInstance?.invalidateSize(); });
...
function updateActivityPreview() {
  ...
  document.getElementById('pCO2').textContent=(value*rate.co2).toFixed(2)+' kg';
  document.getElementById('pPts').textContent=Math.round(value*rate.points)+' pt';
...
async function saveActivity() {
  ...
  btn.disabled=false; btn.innerHTML='<i class="fas fa-save"></i> Salva attività';
  if(data.error) { showNotification(data.error,'error'); return; }
  showNotification(`✅ Attività salvata! +${data.co2_saved} kg CO₂, +${data.points} pt`,'success');
  ['actKm','actHours','actNote','fromAddr','toAddr'].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=''; });
...
async function loadActivities() {
  ...
  const container=document.getElementById('actList'); if(!container) return;
  if(data.error||!data.length) {
    container.innerHTML=`<div class="empty-state"><span>🌱</span><p>Nessuna attività registrata</p></div>`; return;
  }
...
async function searchAddress(fieldId, query) {
  ...
  try {
    ...
    sugg.innerHTML=data.map(p=>`<div class="addr-item" onclick="selectAddress('${fieldId}',${JSON.stringify(p.display_name)})">${escapeHtml(p.display_name)}</div>`).join('');
  } catch {}
}
...
function getUserLocation() {
  ...
  navigator.geolocation.getCurrentPosition(pos=>{
    ...
    fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}`)
      .then(r=>r.json()).then(d=>{ if(d.display_name) document.getElementById('fromAddr').value=d.display_name; }).catch(()=>{});
...
async function calculateRoute() {
  ...
  try {
    ...
    routingControl=L.Routing.control({
      waypoints:[L.latLng(parseFloat(fRes[0].lat),parseFloat(fRes[0].lon)),L.latLng(parseFloat(tRes[0].lat),parseFloat(tRes[0].lon))],
      routeWhileDragging:false, showAlternatives:false, fitSelectedRoutes:true,
      lineOptions:{styles:[{color:'#16a34a',weight:5}]}, createMarker:()=>null
    }).addTo(mapInstance);
    routingControl.on('routesfound',e=>{
      const km=(e.routes[0].summary.totalDistance/1000).toFixed(1);
      document.getElementById('actKm').value=km; updateActivityPreview();
      showNotification(`📍 Distanza: ${km} km`,'success');
    });
    routingControl.on('routingerror',()=>showNotification('Percorso non trovato','error'));
  } catch { showNotification('Errore nel calcolo del percorso','error'); }
}
...
async function loadLeaderboard() {
  ...
  const container=document.getElementById('lbList'); if(!container) return;
  if(data.error||!data.length) { container.innerHTML=`<div class="empty-state"><span>🏆</span><p>Nessun utente</p></div>`; return; }
  container.innerHTML=data.map((u,i)=>{
    const rankClass=i===0?'top1':i===1?'top2':i===2?'top3':'';
    const medals=['🥇','🥈','🥉'];
    return `<div class="leaderboard-item">
      <div class="lb-rank ${rankClass}">${medals[i]||i+1}</div>
      <div class="lb-info"><strong>${escapeHtml(u.name||u.username)}</strong><small>@${escapeHtml(u.username||'')}</small></div>
      <div class="lb-stats"><span class="lb-co2">🌱 ${parseFloat(u.co2_saved||0).toFixed(1)} kg</span><span class="lb-pts">⭐ ${u.points||0} pt</span></div>
    </div>`;
  }).join('');
}
...
async function loadChallenges() {
  ...
  const container=document.getElementById('chList'); if(!container) return;
  if(data.error||!data.length) { container.innerHTML=`<div class="empty-state"><span>🏆</span><p>Nessuna sfida</p></div>`; return; }
  container.innerHTML=data.map(c=>{
    ...
    return `<div class="challenge-item">
      ...
      <div class="challenge-meta">
        <span>🎯 ${c.co2_target} kg CO₂</span><span>⭐ ${c.points_reward} pt</span>
...
async function createChallenge() {
  ...
  const data=await apiRequest('/api/challenges','POST',{title,description:desc,co2_target:co2,points_reward:pts,end_date:endDate,is_public:isPublic});
  if(data.error) { showNotification(data.error,'error'); return; }
...
// FINE BLOCCO 3
// SOCIAL
async function loadSocial() { await Promise.all([loadPosts(), loadUsers()]); }

async function loadPosts() {
  const data=await apiRequest('/api/social/posts');
  const container=document.getElementById('postsList'); if(!container) return;
  if(data.error||!data.length) { container.innerHTML=`<div class="empty-state"><span>📭</span><p>Nessun post ancora</p></div>`; return; }
  container.innerHTML=data.map(p=>{
    ...
    return `<div class="post-card" id="post-${p.id}">
      <div class="post-header">
        ...
        <div class="post-author"><strong>${escapeHtml(p.author_name||'Utente')}</strong>
          <small>@${escapeHtml(p.author_username||'')} · ${timeAgo(p.created_at)}</small></div>
      </div>
      <div class="post-body">${escapeHtml(p.content)}</div>
      ${p.image_url?`<img class="post-image" src="${escapeHtml(p.image_url)}" alt="img" onerror="this.style.display='none'">`:''}
  ...
  data.forEach(p=>{
    ...
    if(canvas) drawMii({color:p.avatar_color||'#16a34a',skin:p.avatar_skin||'#fde68a',eyes:p.avatar_eyes||'normal',mouth:p.avatar_mouth||'smile',hair:p.avatar_hair||'none'},`postAvatar${p.id}`,36);
  });
}
...
async function deletePost(id) {
  showConfirm('Elimina post','Sei sicuro?',async()=>{
    const data=await apiRequest(`/api/social/posts/${id}`,'DELETE');
    if(data.error) { showNotification(data.error,'error'); return; }
    showNotification('Post eliminato','success'); await loadPosts();
  },'🗑️');
}
...
async function loadComments(postId) {
  ...
  const container=document.getElementById(`commentsList-${postId}`); if(!container) return;
...
async function addComment(postId) {
  ...
  if(data.error) { showNotification(data.error,'error'); return; }
  input.value=''; await loadComments(postId);
...
async function loadUsers() {
  ...
  container.innerHTML=data.map(u=>`
    <div class="user-card">
      ...
      <div class="user-card-info"><strong>${escapeHtml(u.name||u.username)}</strong>
        <small>⭐ ${u.points||0} pt · 🌱 ${parseFloat(u.co2_saved||0).toFixed(1)} kg</small></div>
      <button class="btn-sm ${u.following?'red':'green'}" onclick="toggleFollow(${u.id},this)">
        ${u.following?'Segui già':'+ Segui'}</button>
    </div>`).join('');
  data.forEach(u=>drawMii({color:u.avatar_color||'#16a34a',skin:u.avatar_skin||'#fde68a',eyes:u.avatar_eyes||'normal',mouth:u.avatar_mouth||'smile',hair:u.avatar_hair||'none'},`userAv${u.id}`,36));
}
...
async function loadShop() {
  const [items,profile]=await Promise.all([apiRequest('/api/shop'),apiRequest('/api/profile')]);
  if(items.error) return;
  allShopItems=items;
  document.getElementById('shopPoints').textContent=profile.points||0;
  if(profile.owned_items!==undefined) myProfile={...myProfile,...profile};
  renderShop();
}
...
function renderShop() {
  const container=document.getElementById('shopGrid'); if(!container) return;
  const owned=myProfile?.owned_items||[];
  const filtered=currentShopCategory==='all'?allShopItems:allShopItems.filter(i=>i.category===currentShopCategory);
  if(!filtered.length) { container.innerHTML=`<div class="empty-state"><span>🛍️</span><p>Nessun oggetto</p></div>`; return; }
  container.innerHTML=filtered.map(item=>{
    const isOwned=owned.includes(item.id);
    return `<div class="shop-item ${isOwned?'owned':''} ${item.is_rare?'rare':''}" onclick="buyItem(${item.id})">
      ...
      <div class="shop-item-cost ${item.cost===0?'free':''}">${item.cost===0?'Gratis':'⭐ '+item.cost+' pt'}</div>
    </div>`;
  }).join('');
}
...
async function buyItem(itemId) {
  ...
  const item=allShopItems.find(i=>i.id===itemId); if(!item) return;
  showConfirm(`Acquista ${item.name}`,`Costo: ⭐ ${item.cost} punti. Confermi?`,async()=>{
    const data=await apiRequest('/api/shop/buy','POST',{item_id:itemId});
    if(data.error) { showNotification(data.error,'error'); return; }
    showNotification(`✅ Acquistato: ${item.name}!`,'success');
    if(myProfile) { myProfile.points=data.new_points; myProfile.owned_items=data.owned_items; updateSidebar(myProfile); }
...
function syncMiiState(user) {
  if(!user) return;
  miiState.color=user.avatar_color||'#16a34a'; miiState.skin=user.avatar_skin||'#fde68a';
  miiState.eyes=user.avatar_eyes||'normal';    miiState.mouth=user.avatar_mouth||'smile';
...
function drawMii(state, canvasId, size=120) {
  ...
  const cx=size/2, cy=size/2, hr=size*0.33;
  ctx.clearRect(0,0,size,size);
  ctx.beginPath(); ctx.arc(cx,cy,size*0.48,0,Math.PI*2);
  ctx.fillStyle=state.color||'#16a34a'; ctx.fill();
  ctx.fillStyle='#1f2937';
  if(state.hair==='long') { ctx.beginPath(); ctx.ellipse(cx,cy+hr*0.6,hr*0.75,hr*1.1,0,0,Math.PI*2); ctx.fill(); }
  else if(state.hair==='bun') { ctx.beginPath(); ctx.arc(cx,cy-hr*1.1,hr*0.3,0,Math.PI*2); ctx.fill(); }
  ctx.beginPath(); ctx.arc(cx,cy,hr,0,Math.PI*2);
  ctx.fillStyle=state.skin||'#fde68a'; ctx.fill();
  ctx.strokeStyle='rgba(0,0,0,.1)'; ctx.lineWidth=1; ctx.stroke();
  ctx.fillStyle='#1f2937';
  if(state.hair==='short') { ctx.beginPath(); ctx.arc(cx,cy-hr*0.5,hr*0.85,Math.PI,0); ctx.fill(); }
  else if(state.hair==='curly') { for(let i=-1;i<=1;i++) { ctx.beginPath(); ctx.arc(cx+i*hr*0.35,cy-hr*0.85,hr*0.22,0,Math.PI*2); ctx.fill(); } }
  else if(state.hair==='spiky') { for(let i=-2;i<=2;i++) { ctx.beginPath(); ctx.moveTo(cx+i*hr*0.22,cy-hr*0.7); ctx.lineTo(cx+i*hr*0.22-hr*0.12,cy-hr*1.15); ctx.lineTo(cx+i*hr*0.22+hr*0.12,cy-hr*1.15); ctx.closePath(); ctx.fill(); } }
  const eyeY=cy-hr*0.1, eyeX=hr*0.28, eyeS=hr*0.11;
  ctx.fillStyle='#1f2937';
  if(state.eyes==='happy') { [-1,1].forEach(d=>{ctx.beginPath();ctx.arc(cx+d*eyeX,eyeY,eyeS,Math.PI,0);ctx.fill();}); }
  else if(state.eyes==='sleepy') { [-1,1].forEach(d=>{ctx.beginPath();ctx.arc(cx+d*eyeX,eyeY,eyeS,0,Math.PI);ctx.fill();}); }
  else if(state.eyes==='surprised') { [-1,1].forEach(d=>{ctx.beginPath();ctx.arc(cx+d*eyeX,eyeY,eyeS*1.4,0,Math.PI*2);ctx.fill();}); }
    ...
    [-1,1].forEach(d=>{ctx.beginPath();ctx.ellipse(cx+d*eyeX,eyeY,eyeS*1.5,eyeS*0.7,0,0,Math.PI*2);ctx.fill();});
    ctx.fillStyle='#64748b';ctx.fillRect(cx-eyeX*1.6,eyeY-eyeS*1.2,eyeX*3.2,eyeS*0.6);
  } else if(state.eyes==='star') {
    ctx.fillStyle='#f59e0b';
    [-1,1].forEach(d=>drawStar(ctx,cx+d*eyeX,eyeY,5,eyeS*1.2,eyeS*0.5));
  } else if(state.eyes==='heart') {
    ctx.fillStyle='#ef4444';
    [-1,1].forEach(d=>drawHeart(ctx,cx+d*eyeX,eyeY,eyeS*1.1));
  } else {
    [-1,1].forEach(d=>{
      ctx.fillStyle='#1f2937';ctx.beginPath();ctx.arc(cx+d*eyeX,eyeY,eyeS,0,Math.PI*2);ctx.fill();
      ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(cx+d*eyeX-eyeS*0.3,eyeY-eyeS*0.3,eyeS*0.3,0,Math.PI*2);ctx.fill();
    });
  }
  const mouthY=cy+hr*0.35, mouthR=hr*0.22;
  ctx.strokeStyle='#1f2937'; ctx.lineWidth=size*0.025;
  if(state.mouth==='grin') {
    ctx.beginPath();ctx.arc(cx,mouthY,mouthR,0,Math.PI);ctx.fillStyle='#ef4444';ctx.fill();ctx.stroke();
    ctx.fillStyle='#fff';ctx.fillRect(cx-mouthR+2,mouthY-2,mouthR*2-4,6);
  } else if(state.mouth==='open') {
    ctx.beginPath();ctx.ellipse(cx,mouthY,mouthR*0.7,mouthR*0.5,0,0,Math.PI*2);ctx.fillStyle='#ef4444';ctx.fill();ctx.stroke();
  } else if(state.mouth==='smirk') {
    ctx.beginPath();ctx.moveTo(cx-mouthR*0.3,mouthY);ctx.quadraticCurveTo(cx+mouthR*0.3,mouthY-4,cx+mouthR*0.7,mouthY-8);ctx.stroke();
    ...
    ['#ef4444','#f59e0b','#22c55e','#3b82f6','#8b5cf6'].forEach((color,i)=>{
      ctx.strokeStyle=color;ctx.lineWidth=size*0.018;ctx.beginPath();ctx.arc(cx,mouthY,mouthR+i*3,0,Math.PI);ctx.stroke();
    });
  } else { ctx.beginPath();ctx.arc(cx,mouthY,mouthR,0,Math.PI);ctx.stroke(); }
}

function drawStar(ctx,cx,cy,points,r,ir) {
  ctx.beginPath();
  for(let i=0;i<points*2;i++) { const a=(i*Math.PI)/points-Math.PI/2,rd=i%2===0?r:ir; i===0?ctx.moveTo(cx+rd*Math.cos(a),cy+rd*Math.sin(a)):ctx.lineTo(cx+rd*Math.cos(a),cy+rd*Math.sin(a)); }
...
async function loadAvatarSection() {
  ...
  if(colorOpts) colorOpts.innerHTML=BG_COLORS.map(c=>`<div class="color-swatch ${miiState.color===c?'selected':''}" style="background:${c};border:2px solid ${c===' #ffffff'?'#e2e8f0':'transparent'};" onclick="setAvatarColor('${c}',this)"></div>`).join('');
  const skinOpts=document.getElementById('skinOptions');
  if(skinOpts) skinOpts.innerHTML=SKIN_COLORS.map(c=>`<div class="color-swatch ${miiState.skin===c?'selected':''}" style="background:${c};" onclick="setAvatarSkin('${c}',this)"></div>`).join('');
  const hairOpts=document.getElementById('hairOptions');
  if(hairOpts) hairOpts.innerHTML=HAIR_OPTIONS.map(h=>`<button class="option-btn ${miiState.hair===h?'selected':''}" onclick="setAvatarHair('${h}',this)">${h==='none'?'🚫 Nessuno':h==='short'?'💇 Corti':h==='long'?'💁 Lunghi':h==='curly'?'🦱 Ricci':h==='spiky'?'⚡ Spiky':'🎀 Bun'}</button>`).join('');
  const eyeOpts=document.getElementById('eyeOptions');
  if(eyeOpts) eyeOpts.innerHTML=EYE_OPTIONS.map(e=>`<button class="option-btn ${miiState.eyes===e?'selected':''}" onclick="setAvatarEyes('${e}',this)">${e==='normal'?'😐 Normali':e==='happy'?'😊 Felici':e==='sleepy'?'😴 Assonnati':e==='surprised'?'😲 Sorpresi':e==='wink'?'😉 Occhiolino':e==='cool'?'😎 Cool':e==='star'?'⭐ Stella':'❤️ Cuore'}</button>`).join('');
  const mouthOpts=document.getElementById('mouthOptions');
  if(mouthOpts) mouthOpts.innerHTML=MOUTH_OPTIONS.map(m=>`<button class="option-btn ${miiState.mouth===m?'selected':''}" onclick="setAvatarMouth('${m}',this)">${m==='smile'?'😊 Sorriso':m==='grin'?'😁 Ghigno':m==='open'?'😮 Aperta':m==='smirk'?'😏 Smorfia':m==='sad'?'😢 Triste':'🌈 Arcobaleno'}</button>`).join('');
  drawMii(miiState,'miiCanvas',200);
}

function setAvatarColor(color,el) { miiState.color=color; document.querySelectorAll('#colorOptions .color-swatch').forEach(s=>s.classList.remove('selected')); el.classList.add('selected'); drawMii(miiState,'miiCanvas',200); drawMii(miiState,'sidebarAvatar',48); }
window.setAvatarColor=setAvatarColor;
function setAvatarSkin(skin,el) { miiState.skin=skin; document.querySelectorAll('#skinOptions .color-swatch').forEach(s=>s.classList.remove('selected')); el.classList.add('selected'); drawMii(miiState,'miiCanvas',200); drawMii(miiState,'sidebarAvatar',48); }
window.setAvatarSkin=setAvatarSkin;
function setAvatarHair(hair,el) { miiState.hair=hair; document.querySelectorAll('#hairOptions .option-btn').forEach(b=>b.classList.remove('selected')); el.classList.add('selected'); drawMii(miiState,'miiCanvas',200); drawMii(miiState,'sidebarAvatar',48); }
window.setAvatarHair=setAvatarHair;
function setAvatarEyes(eyes,el) { miiState.eyes=eyes; document.querySelectorAll('#eyeOptions .option-btn').forEach(b=>b.classList.remove('selected')); el.classList.add('selected'); drawMii(miiState,'miiCanvas',200); drawMii(miiState,'sidebarAvatar',48); }
window.setAvatarEyes=setAvatarEyes;
function setAvatarMouth(mouth,el) { miiState.mouth=mouth; document.querySelectorAll('#mouthOptions .option-btn').forEach(b=>b.classList.remove('selected')); el.classList.add('selected'); drawMii(miiState,'miiCanvas',200); drawMii(miiState,'sidebarAvatar',48); }
window.setAvatarMouth=setAvatarMouth;

async function saveAvatar() {
  const data=await apiRequest('/api/profile/avatar','PUT',{color:miiState.color,skin:miiState.skin,eyes:miiState.eyes,mouth:miiState.mouth,hair:miiState.hair});
  if(data.error) { showNotification(data.error,'error'); return; }
  if(myProfile) { myProfile.avatar_color=miiState.color;myProfile.avatar_skin=miiState.skin;myProfile.avatar_eyes=miiState.eyes;myProfile.avatar_mouth=miiState.mouth;myProfile.avatar_hair=miiState.hair; }
  showNotification('✅ Avatar salvato!','success'); drawMii(miiState,'sidebarAvatar',48);
}
...
async function loadProfile() {
  ...
  const pts=profile.points||0, lvl=Math.floor(pts/100)+1, next=lvl*100, pct=((pts%100)/100)*100;
  document.getElementById('profLevel').textContent=`Livello ${lvl} 🌱`;
  document.getElementById('xpText').textContent   =`${pts}/${next} XP`;
  document.getElementById('xpBar').style.width    =pct+'%';
  syncMiiState(profile); await loadBadges();
}
window.loadProfile=loadProfile;

async function loadBadges() {
  const badges=await apiRequest('/api/badges');
  const container=document.getElementById('badgeList'); if(!container||badges.error) return;
  container.innerHTML=badges.map(b=>`<div class="badge-item ${b.unlocked?'unlocked':'locked'}" title="${escapeHtml(b.desc)}"><span>${b.icon}</span><strong>${escapeHtml(b.name)}</strong></div>`).join('');
}
...
async function changePassword() {
  ...
  if(!checkPasswordStrength(nw)) { showNotification('Nuova password non sicura','error'); return; }
  const data=await apiRequest('/api/profile/password','PUT',{current_password:cur,new_password:nw});
  if(data.error) { showNotification(data.error,'error'); return; }
  document.getElementById('currentPw').value=''; document.getElementById('newPw').value='';
  showNotification('✅ Password aggiornata!','success');
}
...
async function loadNotifications() {
  ...
  const container=document.getElementById('notifList'); if(!container) return;
  if(data.error||!data.length) { container.innerHTML=`<div class="empty-state"><span>🔔</span><p>Nessuna notifica</p></div>`; return; }
  container.innerHTML=data.map(n=>`
    <div class="notif-item ${n.is_read?'':'unread'}" onclick="markRead(${n.id},this)">
      <span class="notif-icon">${n.icon||'🔔'}</span>
      <div class="notif-body"><p>${escapeHtml(n.message)}</p><small>${timeAgo(n.created_at)}</small></div>
...
async function loadNotificationCount() {
  ...
  if(!data.error&&data.count>0) { badge.textContent=data.count; badge.style.display='inline-block'; }
...
async function loadAdminPanel() {
  ...
  if(!stats.error) {
    document.getElementById('adminStats').innerHTML=`
      ...
      <div class="stat-card purple"><div class="stat-card-icon">🌍</div><div class="stat-card-body"><span class="stat-card-value">${parseFloat(stats.total_co2).toFixed(1)}</span><span class="stat-card-label">kg CO₂</span></div></div>
...
async function loadAdminUsers() {
  ...
  const tbody=document.getElementById('adminUsersList'); if(!tbody||data.error) return;
  tbody.innerHTML=data.map(u=>`<tr>
    <td>${u.id}</td><td>${escapeHtml(u.name||'-')}</td><td>@${escapeHtml(u.username||'-')}</td>
    <td style="font-size:12px;">${escapeHtml(u.email)}</td><td>${u.points||0}</td>
    <td>${parseFloat(u.co2_saved||0).toFixed(1)}</td><td>${u.activity_count||0}</td>
    ...
    <td><div class="admin-actions">
      ...
      ${!u.is_banned?`<button class="btn-sm red" onclick="openBanModal(${u.id})">🔨 Ban</button>`:`<button class="btn-sm green" onclick="unbanUser(${u.id})">✅ Unban</button>`}
...
async function loadAdminActivities() {
  ...
  const tbody=document.getElementById('adminActivitiesList'); if(!tbody||data.error) return;
  tbody.innerHTML=data.map(a=>`<tr>
    <td>${a.id}</td><td>${escapeHtml(a.user_name||'-')}</td><td>${ACTIVITY_ICONS[a.type]||''} ${escapeHtml(a.type)}</td>
    <td>${a.km||0}</td><td>${a.hours||0}</td><td>${a.co2_saved||0}</td><td>${a.points||0}</td>
    <td style="font-size:12px;">${new Date(a.date).toLocaleDateString('it-IT')}</td>
    <td><button class="btn-sm red" onclick="deleteActivity(${a.id})">🗑️ Elimina</button></td></tr>`).join('');
}

async function loadAdminPosts() {
  const data=await apiRequest('/api/admin/posts');
  const tbody=document.getElementById('adminPostsList'); if(!tbody||data.error) return;
  tbody.innerHTML=data.map(p=>{
    const likes=typeof p.likes==='string'?JSON.parse(p.likes):(p.likes||[]);
    return `<tr><td>${p.id}</td><td>${escapeHtml(p.author_name||'-')}</td>
      <td style="max-width:300px;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(p.content)}</td>
      <td>❤️ ${likes.length}</td><td style="font-size:12px;">${new Date(p.created_at).toLocaleDateString('it-IT')}</td>
      <td><button class="btn-sm red" onclick="adminDeletePost(${p.id})">🗑️ Elimina</button></td></tr>`;
  }).join('');
}
...
async function confirmEditUser() {
  ...
  if(data.error) { showNotification(data.error,'error'); return; }
  closeEditUserModal(); showNotification('✅ Utente aggiornato!','success'); await loadAdminUsers();
}
...
async function confirmBan() {
  ...
  if(data.error) { showNotification(data.error,'error'); return; }
  closeBanModal(); showNotification('🔨 Utente bannato','success'); await loadAdminUsers();
}
window.confirmBan=confirmBan;

async function unbanUser(id) {
  showConfirm('Rimuovi ban','Rimuovere il ban?',async()=>{
    const data=await apiRequest(`/api/admin/users/${id}/unban`,'POST');
    if(data.error) { showNotification(data.error,'error'); return; }
    showNotification('✅ Ban rimosso!','success'); await loadAdminUsers();
  },'✅');
}
...
async function deleteUser(id, name) {
  showConfirm('Elimina utente',`Eliminare ${escapeHtml(name)}? Azione irreversibile.`,async()=>{
    ...
    if(data.error) { showNotification(data.error,'error'); return; }
    showNotification('✅ Utente eliminato','success'); await loadAdminUsers();
  },'🗑️');
}
window.deleteUser=deleteUser;

async function deleteActivity(id) {
  showConfirm('Elimina attività','Eliminare? I punti verranno rimossi.',async()=>{
    const data=await apiRequest(`/api/admin/activities/${id}`,'DELETE');
    if(data.error) { showNotification(data.error,'error'); return; }
    showNotification('✅ Attività eliminata','success'); await loadAdminActivities();
  },'🗑️');
}
...
async function adminDeletePost(id) {
  showConfirm('Elimina post','Eliminare questo post?',async()=>{
    ...
    if(data.error) { showNotification(data.error,'error'); return; }
    showNotification('✅ Post eliminato','success'); await loadAdminPosts();
  },'🗑️');
}
...
function showTutorial() { tutorialStep=1; renderTutorialStep(); document.getElementById('tutorialModal').style.display='flex'; }
window.showTutorial=showTutorial;

function renderTutorialStep() {
  const step=TUTORIAL_STEPS[tutorialStep-1];
  document.getElementById('tutorialContent').innerHTML=`<div class="tutorial-step"><div class="tutorial-emoji">${step.emoji}</div><h3>${escapeHtml(step.title)}</h3><p>${escapeHtml(step.text)}</p></div>`;
  document.getElementById('tutorialDots').innerHTML=TUTORIAL_STEPS.map((_,i)=>`<div class="tutorial-dot ${i+1===tutorialStep?'active':''}"></div>`).join('');
  document.getElementById('tutorialNext').textContent=tutorialStep===TUTORIAL_STEPS.length?'🎉 Inizia!':'Avanti →';
}

async function nextTutorialStep() {
  if(tutorialStep<TUTORIAL_STEPS.length) { tutorialStep++; renderTutorialStep(); } else await skipTutorial();
}
...
document.addEventListener('DOMContentLoaded', async()=>{
  const params=new URLSearchParams(window.location.search);
  if(params.get('action')==='reset'&&params.get('token')) switchAuthTab('reset');
  if(params.get('verified')==='1') showNotification('✅ Email verificata! Ora puoi accedere.','success');
  ...
  if(token) {
    ...
    if(!data.error) {
      ...
      document.getElementById('authContainer').style.display='none';
      document.getElementById('appContainer').style.display='flex';
      updateSidebar(data); syncMiiState(data);
      await loadDashboard(); await loadNotificationCount();
      setInterval(loadNotificationCount,30000);
    } else { token=null; localStorage.removeItem('ecotoken'); }
  }
  ...
  document.getElementById('banModal')?.addEventListener('click',e=>{ if(e.target===document.getElementById('banModal')) closeBanModal(); });
  document.getElementById('editUserModal')?.addEventListener('click',e=>{ if(e.target===document.getElementById('editUserModal')) closeEditUserModal(); });
});