...
const actionLimiter = rateLimit(60 * 1000, 15); // Max 15 azioni al minuto per attività, social, ecc.
...
app.post('/api/register', authLimiter, async (req, res) => {
    try {
        ...
        // B2B Fallback: Se la mail non è configurata, auto-approva l'utente per non bloccare l'onboarding
        const isEmailActive = !!process.env.SENDGRID_API_KEY;
        const autoVerified = isEmailActive ? 0 : 1;

        const { rows } = await db.query(
            "INSERT INTO users (name,username,email,password,verify_token,verified,owned_items) VALUES ($1,$2,$3,$4,$5,$6,'[]') RETURNING *",
            [name.trim(), username.toLowerCase(), email.toLowerCase(), hash, vTok, autoVerified]
        );
        ...
        const successMsg = isEmailActive 
            ? 'Registrazione completata! Controlla la tua email.' 
            : 'Registrazione completata! (Auto-verificata)';
...
app.post('/api/activities', auth, actionLimiter, async (req, res) => {
    try {
        ...
        // Anti-cheat aziendale: massimali realistici
        const maxKm = type === 'Bici' ? 150 : type === 'Carpooling' ? 500 : 1500;
        const maxHours = type === 'Remoto' || type === 'Videocall' ? 12 : 24;
        
        const kmVal = Math.min(parseFloat(km) || 0, maxKm);
        const hoursVal = Math.min(parseFloat(hours) || 0, maxHours);
...
app.post('/api/social/posts', auth, actionLimiter, async (req, res) => {
    try {
...
app.post('/api/social/posts/:id/comments', auth, actionLimiter, async (req, res) => {
    try {
