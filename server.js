...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (!user.verified) return res.status(400).json({ error: 'Email non verificata.', needsVerify: true });
...
const BADGES = [
  ...
  { id: 'co2100',  name: '100kg CO₂',    icon: '🏆', desc: '100kg CO₂',            check: (u)       => u.co2_saved >= 100 },
  { id: 'pts100',  name: '100 Punti',    icon: '⭐', desc: '100 punti',             check: (u)       => u.points >= 100   },
  { id: 'pts500',  name: '500 Punti',    icon: '🌟', desc: '500 punti',             check: (u)       => u.points >= 500   },
  { id: 'pts1000', name: '1000 Punti',   icon: '💫', desc: '1000 punti',            check: (u)       => u.points >= 1000  },
  { id: 'social1', name: 'Social Start', icon: '📣', desc: 'Primo post',            check: (u,a,p)   => p >= 1    },
...
app.post('/api/shop/buy', auth, async (req, res) => {
  try {
    ...
    owned.push(item_id);
    await db.query('UPDATE users SET points=points-$1,owned_items=$2 WHERE id=$3', [item.cost, JSON.stringify(owned), req.user.id]);
    await db.query(
...
app.put('/api/admin/users/:id', adminAuth, async (req, res) => {
  try {
    ...
    const { rows: ex } = await db.query('SELECT id FROM users WHERE username=$1 AND id!=$2', [username.toLowerCase(), req.params.id]);
    if (ex.length) return res.status(400).json({ error: 'Username già in uso' });
    await db.query(
...
app.post('/api/admin/users/:id/ban', adminAuth, async (req, res) => {
  try {
    ...
    if (parseInt(req.params.id) === req.user.id) return res.status(400).json({ error: 'Non puoi bannare te stesso' });
